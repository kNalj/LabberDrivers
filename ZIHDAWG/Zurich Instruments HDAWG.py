...
class Driver(LabberDriver):
    ...
    def _configure_sequencer(self, group=1, buffer_size=None):
        ...
        if self.getValue('Channel grouping') != '1 x 4':
            raise Error('Driver currently only support 1x4 channel group mode')
    ...
    def _upload_waveforms(self, awg_updated=None):
        ...
        for ch in range(0, self.n_ch, 2):
            ...
            if awg_updated[ch] or awg_updated[ch + 1]:
                ...
                # self.daq.setInt(base + 'waveform/index', 0)  # This node was deprecated
                self.daq.sync()
                # self.daq.vectorWrite(base + 'waveform/waves/0', data.flatten())  # was replaced by the next line cmd
                self.daq.setVector(base + 'waveform/waves/0', data.flatten())
                # set enabled
                self.daq.setInt(base + 'enable', 1)
